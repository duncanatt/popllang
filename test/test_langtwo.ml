...
let compare_reduce_all (input: string) (expected: Ast.expr): test = 
  "[" ^ input ^ "]" >:: fun _ ->  assert_equal (Sem.reduce_all (Run.get_ast input)) expected
...
let tests: test list = [
  ...
  compare_ast "fun f(x: num){ x + 1 }" (Fun ("f", ("x", Some TNum), BinOp (Add, Var "x", Val (Num 1))));
  compare_ast "fun f(x: num, y: bool){ if y then x + 1 else x - 1 }" 
    (Fun ("f", ("x", Some TNum), FunAnon (("y", Some TBool), If (Var "y", BinOp (Add, Var "x", Val (Num 1)), BinOp (Sub, Var "x", Val (Num 1))))));
  compare_ast "let x: num = 5 in x + 1" (App (FunAnon (("x", Some TNum), BinOp (Add, Var "x", Val (Num 1))), Val (Num 5)));
  compare_ast "let x: num = 5, y: bool = false in if y then x + 1 else x - 1" 
  ...
  compare_ast "fun f(x: num -> bool -> num){ x 1 true }" 
  ...
  compare_ast "fun f(x1, x2){ let y = 3 in x1 + x2 + y }" (Fun ("f", ("x1", None), FunAnon (("x2", None), App (FunAnon (("y", None), BinOp (Add, BinOp (Add, Var "x1", Var "x2"), Var "y")), Val (Num 3)))));
  ...
  (* Compare reduce all *)
  compare_reduce_all "1" (Val (Num 1));
  compare_reduce_all
    "let x = 1
       in let f = fun(y){y + x} 
         in let x = 2
           in f 3" 
    (Val (Num 4));
  compare_reduce_all "(fun (x){fun (y){ x + y }} (1 + 2)) (3 + 4)" (Val (Num 10));
  compare_reduce_all 
    "let succ : num = fun(x){x+1} 
  ...
  compare_reduce_all
    "if true then
        ...
        fun g(y){ y 2 }"
    (Fun ("f", ("x", None), App (FunAnon (("y", None), BinOp (Add, Var "x", Var "y")), Val (Num 3))));
]
